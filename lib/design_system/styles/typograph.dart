import 'package:flutter/material.dart';

class TypographyCollection {
  static const TextStyle h1 = TextStyle(
      fontFamily: 'Quicksand', fontSize: 24, fontWeight: FontWeight.bold);
  static const TextStyle h2 = TextStyle(
      fontFamily: 'Quicksand', fontSize: 20, fontWeight: FontWeight.bold);
  static const TextStyle h3 = TextStyle(
      fontFamily: 'Quicksand', fontSize: 16, fontWeight: FontWeight.bold);
  static const TextStyle h4 = TextStyle(
      fontFamily: 'Quicksand', fontSize: 14, fontWeight: FontWeight.bold);
  static const TextStyle h5 = TextStyle(
      fontFamily: 'Quicksand', fontSize: 12, fontWeight: FontWeight.bold);

  static const TextStyle sh1 = TextStyle(
      fontFamily: 'Quicksand', fontSize: 20, fontWeight: FontWeight.w700);
  static const TextStyle sh2 = TextStyle(
      fontFamily: 'Quicksand', fontSize: 16, fontWeight: FontWeight.w700);
  static const TextStyle sh3 = TextStyle(
      fontFamily: 'Quicksand', fontSize: 14, fontWeight: FontWeight.w700);
  static const TextStyle sh4 = TextStyle(
      fontFamily: 'Quicksand', fontSize: 12, fontWeight: FontWeight.w700);

  static const TextStyle p1 = TextStyle(
      fontFamily: 'Quicksand', fontSize: 16, fontWeight: FontWeight.w500);
  static const TextStyle p2 = TextStyle(
      fontFamily: 'Quicksand', fontSize: 14, fontWeight: FontWeight.w500);
  static const TextStyle p3 = TextStyle(
      fontFamily: 'Quicksand', fontSize: 12, fontWeight: FontWeight.w500);
}
